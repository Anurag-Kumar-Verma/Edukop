...
import { IComment } from "src/app/model/IComment.model";
import { IGroupProduct } from "src/app/model/IGProduct.model";
import { INews } from "src/app/model/INews.model";
import { IProduct } from "src/app/model/product.model";
import { SearchDataModel } from "src/app/product-detail/product-detail.component";
import { SearchModel } from "src/app/product-list/product-list.component";
...
export class SharedService {
  ...
  ): Observable<interfaces.IResponsePaginationGet<IProduct[]>> {
    return this.http.post<
      interfaces.IResponsePaginationGet<IProduct[]>
    >(
      environment.Api +
  ...
  getProductById(
    ...
    IProduct | IGroupProduct | interfaces.IVariantProduct
  > {
  ...
  getCommentCount(news_uuid: string) {
    return this.http.get<any>(
  ...
  searchProductByDiscount(
    ...
    return this.http.get<interfaces.IResponsePaginationGet<IProduct[]>>(
      environment.Api +
  ...
  getUserInfo(): Observable<interfaces.IResponseGet<any>> {
    return this.http.get<interfaces.IResponseGet<any>>(
  ...
  getVendor(
    uuid: string
  ): Observable<interfaces.IResponseGet<Array<interfaces.IVendorInventory>>> {
    return this.http.get<
      interfaces.IResponseGet<Array<interfaces.IVendorInventory>>
  ...
  uploadProfileImage(form: File): Observable<any> {
    const formData: FormData = new FormData();
    formData.append("imagePath", form);
    return this.http.post<any>(environment.Api + `/userImage`, formData);
  }

  uploadProductImage(form: File, resUUID: string): Observable<any> {
    const formData: FormData = new FormData();
    formData.append("imagePath", form);
    return this.http.post<any>(
      environment.Api + `/request2?uuid=${resUUID}`,
      formData
    );
  }
  ...
  exportFile(res: Blob, fileName: string) {
    console.log(res);
